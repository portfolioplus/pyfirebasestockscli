...
class TestCLI(unittest.TestCase):
    ...
    def test_2_create_fastmode(self):
        ...
        # check for all keys
        assert all(map(lambda x: "tags" in x and "type" in x, tag_docs))
        # check for all types
        assert all(
            map(
        ...
        assert all(
            map(lambda x: all(map(lambda y: y in x, STOCK_ATTR)), stocks_doc)
        )
    ...
    def test_4_tag_export(self):
        ...
        with open(json_file) as json_file:
            ...
            assert all(
                map(
                    lambda x: x in data, ["indices", "countries", "industries"]
                )
            )
